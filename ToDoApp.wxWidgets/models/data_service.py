        ...
        for idx, item_data in enumerate(items_data):
            try:
                if not isinstance(item_data, dict):
                    wx.LogError(f"Skipping invalid item at index {idx}: expected dict, got {type(item_data)}")
                    continue
                items.append(TodoItem.from_dict(item_data))
            except (ValueError, KeyError, TypeError) as e:
                wx.LogError(f"Failed to load item at index {idx}: {e}")
                continue
...
class DataService:
        ...
        try:
            ...
            if self._data_file.exists():
                ...
                with open(self._data_file, "r", encoding="utf-8") as f:
                    ...
                    if not isinstance(data, dict):
                        wx.LogError(f"Invalid data format: expected dict, got {type(data)}")
                        return ProjectData()
    ...
    def save_data(self, data: ProjectData) -> None:
        """Save data to file."""
        try:
            self._ensure_data_dir()
            ...
            wx.LogError(f"Failed to save data (IO error): {e}")
            wx.MessageBox(
                f"データの保存に失敗しました: {e}",
                "エラー",
                wx.OK | wx.ICON_ERROR
    ...
    def export_data(self, data: ProjectData, parent: Optional[wx.Window] = None) -> bool:
        ...
        with wx.FileDialog(
            ...
            try:
                ...
                wx.LogError(f"Failed to export data: {e}")
                wx.MessageBox(
                    f"エクスポートに失敗しました: {e}",
                    "エラー",
                    wx.OK | wx.ICON_ERROR
